...
withoutmissingDS=xlsread('withoutmissing');
% Loading dataset
tic
load cdataset.mat
[N , d]= size(cdataset);
...
cdataset=dataMatrixImput;
% cdataset(cdataset<0)=0;

% CDATASET=imputed dataset without nondigit cells by tensor
CDATASET = cdataset;
% preparing for bayesian impute
...
cdataset = C;
cdataset(:,[1:5,7:end])= CDATASET;
cdataset = max(cdataset,0);
convertedDS = convert2categoricalvalues(cdataset,withoutmissingDS);
% % SMOTE
% [final_features final_mark] = SMOTE(cdataset,cTarget);
